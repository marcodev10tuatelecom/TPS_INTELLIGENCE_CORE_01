/*=============================================================================
 ...
 @environment       PRODUCTION — SYNTHETIC ROLLBACK-ONLY TEST; REQUIRES EXPLICIT TEST GATE
 @purpose           Exercise the real PL/SQL vertical slice end-to-end with synthetic rows:
                    schedule DML, overlap rejection, rights fail-closed, activation/current item,
                    D3KA network fallback, immutable continuity ledger, AI permission guard,
                    and bounded AI schedule insertion.
 @persistence       SAVEPOINT + ROLLBACK TO SAVEPOINT on PASS and on any exception.
 @expected          PRG-900=PASS and no synthetic rows persist.
 @warning           This source performs temporary DML when executed. Do not run merely to inspect it.
=============================================================================*/
...
BEGIN
    ...
    VALUES(l_type_id, l_tag || '_OWNER', 'Synthetic Affiliate Owner')
    RETURNING entity_id INTO l_owner_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_NETWORK', 'Synthetic Parent Network')
    RETURNING entity_id INTO l_network_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_LIVE', 'Synthetic Live Program')
    RETURNING entity_id INTO l_live_content_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_FILE', 'Synthetic Playable File')
    RETURNING entity_id INTO l_file_content_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_NO_RIGHTS', 'Synthetic Content Without Rights')
    RETURNING entity_id INTO l_no_rights_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_ASSET', 'Synthetic File Asset')
    RETURNING entity_id INTO l_asset_entity_id;

    INSERT INTO tps_entity(entity_type_id, canonical_key, canonical_name)
    VALUES(l_type_id, l_tag || '_ASSET2', 'Synthetic No-Rights Asset')
    RETURNING entity_id INTO l_asset2_entity_id;
    ...
    INSERT INTO tps_media_asset(
        ...
        l_asset_entity_id, l_file_content_id, RPAD('A',64,'A'), 'video/mp4',
    ...
    INSERT INTO tps_media_asset(
        ...
        l_asset2_entity_id, l_no_rights_id, RPAD('B',64,'B'), 'video/mp4',
        'H264','AAC',1800000,1000000,'tps://synthetic/' || l_tag || '/norights.mp4','ACTIVE'
    );

    /* Rights for local live content, and file content for both network and local owner. */
    ...
    l_local_schedule_id := tps_programming_pkg.create_schedule(
        p_schedule_key    => l_tag || '_LOCAL',
        p_owner_entity_id => l_owner_id,
        p_timezone_name   => 'America/Sao_Paulo',
        p_schedule_class  => 'STATION',
        p_valid_from      => l_now - INTERVAL '1' HOUR,
        p_valid_to        => l_now + INTERVAL '2' HOUR,
        p_precedence      => 10
    );
    ...
    /* Exact overlap must be rejected before any second row is inserted. */
    BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLCODE <> -20215 THEN
                RAISE;
            END IF;
    END;

    l_report := tps_programming_pkg.validation_report(l_local_schedule_id);
    SELECT JSON_VALUE(l_report, '$.valid' RETURNING NUMBER) INTO l_valid FROM dual;
    assert_true(l_valid = 1, 'LOCAL SCHEDULE VALIDATION SHOULD PASS');
    ...
    /* Rights fail-closed: asset exists but no ALLOW grant. */
    ...
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLCODE <> -20217 THEN
                RAISE;
            END IF;
    END;

    /* ---------- D3KA parent-network continuity ---------- */
    BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO tps_relation_type(
                ...
                'AFFILIATED_WITH','Affiliated With',0,0,0,0,'ACTIVE'
    ...
    INSERT INTO tps_relation(
        ...
        provenance_source_id, confidence, assertion_class,
        valid_from, state
    ) VALUES(
    ...
    /* Primary local source is DOWN. Local current item is LIVE, so it must be skipped;
       D3KA affiliation should resolve the playable network item. */
    ...
    assert_true(l_decision_code = 'NETWORK_SCHEDULE', 'NETWORK_SCHEDULE DECISION EXPECTED');

    /* Committed-style ledger semantics are enforced even inside this rollback-only test transaction. */
    BEGIN
    ...
    /* ---------- AI capability guard + bounded execution ---------- */
    ...
    INSERT INTO tps_ai_agent(
        ...
        l_tag || '_AGENT','Synthetic Bounded Programming Agent',l_model_id,'BOUNDED_AUTOMATION','ACTIVE'
    ...
    BEGIN
        ...
        assert_true(l_tool_state = 'ACTIVE', 'CANONICAL TPS_PROGRAMMING_TOOL EXISTS BUT IS NOT ACTIVE');
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
    ...
    assert_true(
        tps_ai_guard_pkg.permission_allowed(l_agent_id,'TPS_PROGRAMMING_TOOL','EXECUTE_BOUNDED',l_now) = 1,
    ...
    l_ai_item_id := tps_ai_programming_tool_pkg.execute_bounded_add_item(
        ...
        p_content_entity_id => l_file_content_id,
        p_start_at          => l_now + INTERVAL '130' MINUTE,
        p_end_at            => l_now + INTERVAL '150' MINUTE,
        p_item_class        => 'CONTENT',
        p_priority          => 50,
        p_confidence        => 0.95
    );

    assert_true(l_ai_item_id IS NOT NULL, 'BOUNDED AI ITEM INSERT SHOULD RETURN ID');

    SELECT COUNT(*)
      INTO l_ai_decision_count
    ...
    assert_true(l_ai_decision_count = 1, 'SUCCESSFUL BOUNDED AI ACTION MUST BE RECORDED');
